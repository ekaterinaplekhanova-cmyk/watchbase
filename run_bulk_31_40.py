...
rows = all_rows[30:40]
summary = []
for idx, row in enumerate(rows, start=31):
...
out = PROJECT_ROOT / "output" / "partial_summary_31_40.json"
out.parent.mkdir(parents=True, exist_ok=True)
json.dump(summary, open(out, "w", encoding="utf-8"), ensure_ascii=False, indent=2)
print(f"Saved {out}")

from collections import Counter
print("tiers:", dict(Counter(s["source_tier"] for s in summary)))
print("statuses:", dict(Counter(s["confidence_status"] for s in summary)))
